...
export const ScenarioCard = ({
  ...
  return <div className="h-full bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col">
      {/* Scenario image - optimized for sidebar layout */}
      <div style={{
      backgroundImage: `url(${scenario.imageUrl})`
    }} className="h-84 lg:h-80 bg-cover bg-center relative flex-shrink-0">
      ...
      <div className="flex-1 p-8 lg:p-10 flex flex-col overflow-y-auto">
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="space-y-4 lg:space-y-6 flex-1">
            {scenario.choices.map((choice, index) => <button key={choice.id} onClick={() => onChoiceSelect(choice.id)} className={`w-full p-6 lg:p-8 rounded-2xl font-semibold text-left transition-all duration-300 transform hover:scale-102 shadow-xl active:scale-98 ${impactColors[choice.impact]} min-h-[100px] lg:min-h-[120px]`}>
