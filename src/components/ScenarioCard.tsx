...
export const ScenarioCard = ({ scenario, language, onChoiceSelect }: ScenarioCardProps) => {
  return (
    <div className="h-full bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col">
      ...
      <div className="h-64 lg:h-80 bg-cover relative flex-shrink-0" style={{ backgroundImage: `url(${scenario.imageUrl})`, backgroundPosition: 'center 30%' }}>
        <div className="absolute bottom-6 left-8 right-8">
          <h2 className="text-3xl lg:text-4xl xl:text-5xl font-bold text-black mb-3 drop-shadow-lg">{scenario.title}</h2>
        </div>
      </div>
